...
class GEMMBench(BenchmarkBase):
    ...
    def run(self, args: GEMMBenchArgs):
        ...
        fp32_a = torch.randn(args.m, args.k, device=self.device, dtype=dtype_fp32)
        fp32_b = torch.randn(args.n, args.k, device=self.device, dtype=dtype_fp32).transpose(-1, -2)
        ...
        with torch.inference_mode():
            ...
            ms_fp16 = bench_timer(lambda: torch.matmul(fp16_a, fp16_b), warmups=args.warmups, repeats=args.repeats, 
                                  device=self.device, return_mode=args.return_mode, timeout=args.timeout)
            time.sleep(iterval)
            ms_fp32 = bench_timer(lambda: torch.matmul(fp32_a, fp32_b), warmups=args.warmups, repeats=args.repeats,
                                  device=self.device, return_mode=args.return_mode, timeout=args.timeout)

        tflops_bf16 = nFLOPS / ms_bf16[0] * 1e-9
        tflops_fp16 = nFLOPS / ms_fp16[0] * 1e-9
        tflops_fp32 = nFLOPS / ms_fp32[0] * 1e-9

        self.run_args.append(args)
        self.results = torch.cat([self.results, torch.tensor([tflops_bf16, tflops_fp16, tflops_fp32], device=self.device)])
        dist.barrier()
    ...
    def print_header(self):
        if self.rank == 0:
            ...
            print("Results: (TFLOPS/TOPS)", flush=True)
    ...
    def print_result(self):
        ...
        if self.rank == 0:
            ...
            for args in self.run_args:
                ...
                header.append(f"{args.m}x{args.n}x{args.k}-fp32")
