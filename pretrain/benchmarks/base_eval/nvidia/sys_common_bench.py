...
from aurora.common.metrics.p2p_bandwidth_metrics import P2PBandwidthPerformanceMetrics, P2PMetrics
...
if __name__ == "__main__":
    ...
    if args.aurora_test_type == "gemm/matmul":
        ...
        if data_client is not None:
            ...
            for idx, res in enumerate(all_res):
                ...
                fp32_metrics = GemmOperatorPerformanceMetrics(
                    rank=idx,
                    precision=Precision.FP32.value,
                    operator=Operator.MATMUL.value,
                    warmup_times=args.aurora_warmup_times,
                    repeat_times=args.aurora_repeat_times,
                    shape1_tflops=res[2],
                ...
                data_client.send_data(DataTopic.GEMM_OP_PERFORMANCE, fp32_metrics)
    ...
    elif args.aurora_test_type == "p2p/bandwidth":
        p2p_args = [
            P2PBandwidthLatencyArgs(128, args.aurora_warmup_times, args.aurora_repeat_times),
            P2PBandwidthLatencyArgs(512, args.aurora_warmup_times, args.aurora_repeat_times),
        ]
        bench = P2PBandwidthLatencyBench()
        for case_inputs in p2p_args:
            bench.run(case_inputs)
        bench.print_header()
        bench.print_result()
        ...
        if data_client is not None:
            all_res = bench.results.tolist()
            for idx, run_arg in enumerate(bench.run_args):
                unbid_bw_res = all_res[idx * 3 + 0]
                bid_bw_res = all_res[idx * 3 + 1]
                lat_res = all_res[idx * 3 + 2]

                for gpu_rank in range(len(unbid_bw_res) // bench.device_count):
                    p2p_metrics = P2PBandwidthPerformanceMetrics(
                        rank=gpu_rank,
                        warmup_times=run_arg.warmups,
                        repeat_times=run_arg.repeats,
                        size_in_mb=run_arg.size_in_mb,
                        p2p_metric=P2PMetrics.unidirectional_bandwidth.value,
                        gpu_0=unbid_bw_res[gpu_rank * bench.device_count + 0] if bench.device_count > 0 else None,
                        gpu_1=unbid_bw_res[gpu_rank * bench.device_count + 1] if bench.device_count > 1 else None,
                        gpu_2=unbid_bw_res[gpu_rank * bench.device_count + 2] if bench.device_count > 2 else None,
                        gpu_3=unbid_bw_res[gpu_rank * bench.device_count + 3] if bench.device_count > 3 else None,
                        gpu_4=unbid_bw_res[gpu_rank * bench.device_count + 4] if bench.device_count > 4 else None,
                        gpu_5=unbid_bw_res[gpu_rank * bench.device_count + 5] if bench.device_count > 5 else None,
                        gpu_6=unbid_bw_res[gpu_rank * bench.device_count + 6] if bench.device_count > 6 else None,
                        gpu_7=unbid_bw_res[gpu_rank * bench.device_count + 7] if bench.device_count > 7 else None,
                    )

                    data_client.send_data(DataTopic.P2P_BANDWIDTH_PERFORMANCE, p2p_metrics)
                    p2p_metrics = P2PBandwidthPerformanceMetrics(
                        rank=gpu_rank,
                        warmup_times=run_arg.warmups,
                        repeat_times=run_arg.repeats,
                        size_in_mb=run_arg.size_in_mb,
                        p2p_metric=P2PMetrics.bidirectional_bandwidth.value,
                        gpu_0=bid_bw_res[gpu_rank * bench.device_count + 0] if bench.device_count > 0 else None,
                        gpu_1=bid_bw_res[gpu_rank * bench.device_count + 1] if bench.device_count > 1 else None,
                        gpu_2=bid_bw_res[gpu_rank * bench.device_count + 2] if bench.device_count > 2 else None,
                        gpu_3=bid_bw_res[gpu_rank * bench.device_count + 3] if bench.device_count > 3 else None,
                        gpu_4=bid_bw_res[gpu_rank * bench.device_count + 4] if bench.device_count > 4 else None,
                        gpu_5=bid_bw_res[gpu_rank * bench.device_count + 5] if bench.device_count > 5 else None,
                        gpu_6=bid_bw_res[gpu_rank * bench.device_count + 6] if bench.device_count > 6 else None,
                        gpu_7=bid_bw_res[gpu_rank * bench.device_count + 7] if bench.device_count > 7 else None,
                    )
                    ...
                    p2p_metrics = P2PBandwidthPerformanceMetrics(
                        ...
                        repeat_times=run_arg.repeats,
                        size_in_mb=run_arg.size_in_mb,
                        p2p_metric=P2PMetrics.latency.value,
                        gpu_0=lat_res[gpu_rank * bench.device_count + 0] if bench.device_count > 0 else None,
                        gpu_1=lat_res[gpu_rank * bench.device_count + 1] if bench.device_count > 1 else None,
                        gpu_2=lat_res[gpu_rank * bench.device_count + 2] if bench.device_count > 2 else None,
                        gpu_3=lat_res[gpu_rank * bench.device_count + 3] if bench.device_count > 3 else None,
                        gpu_4=lat_res[gpu_rank * bench.device_count + 4] if bench.device_count > 4 else None,
                        gpu_5=lat_res[gpu_rank * bench.device_count + 5] if bench.device_count > 5 else None,
                        gpu_6=lat_res[gpu_rank * bench.device_count + 6] if bench.device_count > 6 else None,
                        gpu_7=lat_res[gpu_rank * bench.device_count + 7] if bench.device_count > 7 else None,
                    )
                    data_client.send_data(DataTopic.P2P_BANDWIDTH_PERFORMANCE, p2p_metrics)
